...
bool JSXPathNSResolverPrototype::getOwnPropertySlot(JSCell* cell, ExecState* exec, const Identifier& propertyName, PropertySlot& slot)
{
...
bool JSXPathNSResolverPrototype::getOwnPropertyDescriptor(JSObject* object, ExecState* exec, const Identifier& propertyName, PropertyDescriptor& descriptor)
{
...
{
    JSXPathNSResolver* thisObject = jsCast<JSXPathNSResolver*>(cell);
...
EncodedJSValue JSC_HOST_CALL jsXPathNSResolverPrototypeFunctionLookupNamespaceURI(ExecState* exec)
{
    ...
    const String& prefix(ustringToString(MAYBE_MISSING_PARAMETER(exec, 0, DefaultIsUndefined).isEmpty() ? UString() : MAYBE_MISSING_PARAMETER(exec, 0, DefaultIsUndefined).toString(exec)->value(exec)));
...
{
    JSXPathNSResolver* jsXPathNSResolver = static_cast<JSXPathNSResolver*>(handle.get().asCell());
